...
let proj = "+proj=sterea +lat_0=52.15616055555555 +lon_0=5.38763888888889 +k=0.9999079 +x_0=155000 +y_0=463000 +ellps=bessel +towgs84=565.417,50.3319,465.552,-0.398957,0.343988,-1.8774,4.0725 +units=m +no_defs";
    ...
    ws.on('message', (message) => {
        try {
            ...
            if ('lat' in m && 'lon' in m) {
                m['RD-X'] = proj4(proj, [m['lat'], m['lon']])[0];
                m['RD-Y'] = proj4(proj, [m['lat'], m['lon']])[1];
            }
...
app.get('/api/lladder.json', (req, res) => {
    rp(decodeURI(req.query.url)).then(data => {
        ...
        Object.keys(data.scheduledStopPoints).forEach(k => {
            ...
            const RD = new Cartesian(data.scheduledStopPoints[k].x, data.scheduledStopPoints[k].y, 1.0000);
            const ETRS = Transform.rdnap2etrs(RD);

            data.scheduledStopPoints[k].lat = ETRS.phi;
            data.scheduledStopPoints[k].lon = ETRS.lambda;
        });
